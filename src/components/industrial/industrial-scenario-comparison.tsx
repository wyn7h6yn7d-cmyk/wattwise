...
function ScenarioSavingsChart({ comparison }: { comparison: IndustrialScenarioComparison }) {
  ...
  return (
    <ChartCard
      title="Aastane sääst stsenaariumite lõikes"
      description="Lihtsustatud võrdlus samade sisendite põhjal."
      className="!rounded-none border-zinc-800 bg-zinc-950"
      chartClassName="min-h-[180px] md:min-h-[220px]"
    >
      <svg viewBox={`0 0 ${w} ${h}`} className="h-[180px] w-full md:h-[220px]" role="img" aria-label="Säästu võrdlus">
        {[0, 0.5, 1].map((g) => {
  ...
  return (
    <article className="card">
      ...
      <p className="mt-2 text-sm text-zinc-400">
        Võrdleb baasstsenaariumi, ainult PV-d ning PV+aku omatarbe ja peak shaving režiime samade sisenditega.
      </p>
        ...
        <table className="min-w-full text-left text-sm text-zinc-300">
          <thead className="bg-zinc-950 text-xs uppercase tracking-wide text-zinc-500">
            <tr>
              ...
              <th className="px-3 py-2 font-medium">PV toodang MWh</th>
              <th className="px-3 py-2 font-medium">Kohapealne kasutus MWh</th>
              <th className="px-3 py-2 font-medium">Võrku jääv PV MWh</th>
              <th className="px-3 py-2 font-medium">Omatarbe osakaal %</th>
              <th className="px-3 py-2 font-medium">Tipukoormus pärast kW</th>
              <th className="px-3 py-2 font-medium">Aastane sääst €</th>
              <th className="px-3 py-2 font-medium">Lihtsustatud tasuvus a</th>
            </tr>
          </thead>
          <tbody>
            {comparison.scenarios.map((row) => (
              <tr key={row.id} className="border-t border-zinc-800">
                ...
                <td className="px-3 py-2 font-mono tabular-nums">{fmt(row.pvProductionMwh, 1)}</td>
                ...
                <td className="px-3 py-2 font-mono tabular-nums">{fmt(row.selfConsumptionSharePercent, 0)}</td>
      ...
      <div className="mt-4 border border-zinc-800 bg-zinc-950 p-4 text-sm text-zinc-300">
        ...
        <ul className="mt-2 list-disc space-y-1 pl-5">
          <li>
            Suurim sääst: <span className="text-zinc-100">{comparison.conclusion.bestSavingsLabel}</span>
          </li>
          <li>
          ...
          <li>
            ...
            <span className="text-zinc-100">{comparison.conclusion.bestSelfConsumptionLabel}</span>
          </li>
        </ul>
